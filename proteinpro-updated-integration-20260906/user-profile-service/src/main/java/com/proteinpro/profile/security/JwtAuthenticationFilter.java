...
import com.proteinpro.profile.web.ApiErrorResponse;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (authorization == null || !authorization.startsWith("Bearer ")) {
            unauthorized(request, response, "A valid Bearer token is required");
            return;
        }
        try {
            ...
            unauthorized(request, response, "The Bearer token is invalid or expired");
        }
    }
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return path.startsWith("/actuator/health")
                || (request.getMethod().equals("POST") && path.equals("/api/profiles/register"));
    }
    ...
    private void unauthorized(HttpServletRequest request, HttpServletResponse response,
        ...
        objectMapper.writeValue(response.getWriter(), new ApiErrorResponse(
                Instant.now().toString(), 401, "Unauthorized", message, request.getRequestURI()));
    }
}
